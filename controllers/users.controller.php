		...
		if (isset($_POST["newUser"])) {
			...
			if (preg_match('/^[a-zA-Z0-9 ]+$/', $_POST["newName"]) &&
				...
				if ($answer == 'ok') {
						...
						swal({
							type: "success",
							title: "User has been added succesfully!",
				...
				echo '<script>
					...
					swal({
						type: "error",
						title: "Special characters or blank fields are not allowed",
						showConfirmButton: true,
						confirmButtonText: "Close"
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["EditName"])){
				...
				if ($answer == 'ok') {
					...
					echo '<script>
						...
						swal({
							type: "error",
							title: "No special characters in the name or blank field are allowed",
							showConfirmButton: true,
							confirmButtonText: "Close"
							 }).then(function(result){
